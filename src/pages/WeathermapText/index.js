...
const Weathermap = ({ inputCountry, inputCity }) => {
  ...
  useEffect(() => {
    async function fetchData() {
      const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${inputCity},${inputCountry}&appid=${process.env.APIKEYWEATHER}`);
      ...
      <Container>
        {weatherApiData.map((dataOfQuery) =>
          <Card key={dataOfQuery.id} style={{ width: '18rem' }} className="text-center mt-3 p-2">
            ...
            <Card.Body>
              <Card.Title>{inputCity}, {inputCountry}</Card.Title>
              <Card.Text>
